  ...
  <div class="page-container">
    ...
    <div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
      <el-form :inline="true" :model="filters" :size="size">
        ...
        <el-form-item>
          <kt-button :label="$t('action.search')" icon="fa fa-search" perms="sys:dept:view" type="primary" @click="findTreeData(null)"/>
        </el-form-item>
        <el-form-item>
          <kt-button :label="$t('action.add')" icon="fa fa-plus" perms="sys:dept:add" type="primary" @click="handleAdd"/>
        </el-form-item>
      </el-form>
    </div>
    <!--表格树内容栏-->
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <kt-button :label="$t('action.edit')" icon="fa fa-edit" perms="sys:dept:edit" @click="handleEdit(scope.row)"/>
          <kt-button :label="$t('action.delete')" icon="fa fa-trash" perms="sys:dept:delete" type="danger" @click="handleDelete(scope.row)"/>
        </template>
      </el-table-column>
    </el-table>
    <!-- 新增修改界面 -->
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            this.$api.dept.save(params).then((res) => {
              ...
              if (res.code === 200) {
                this.$message.success({ message: '操作成功', type: 'success' })
